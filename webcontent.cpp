  ...
  ui(new Ui::webContent)
{
  ...
  QObject::connect(ui->closeButton, SIGNAL(clicked(bool)), EurThread, SLOT(quit()));
  EurThread->start();
  ...
  QObject::connect(ui->closeButton, SIGNAL(clicked(bool)), UsdThread, SLOT(quit()));
  UsdThread->start();
  ...
  QObject::connect(ui->closeButton, SIGNAL(clicked(bool)), TwdThread, SLOT(quit()));
  TwdThread->start();
...
{
  delete ui;
  //EurContent->deleteLater();
  //UsdContent->deleteLater();
  //TwdContent->deleteLater();
  delete EurThread;
  delete UsdThread;
  delete TwdThread;
  delete EurContent;
